...
  类名存入firstUserFile，最终返回的样例类CallSite存储了最短栈和长度默认为20的最长栈的样例类
  ...
  private[spark] def createSparkEnv(
   ...
   ShuffleManager，useLegacyMemoryManager，BlockManager，MetricsSystem, TempDir等的创建
     */
  ...
  try {
    ...
    // Add each JAR given through the constructor.must be a file but not dir
    if (jars != null) {
      jars.foreach(addJar)
    }
    ...
    // askScheduler、schedulerBackend、dagScheduler创建完成后，向rpc心跳信息接收服务发送一条信息
    ...
    //  blockmanager 初始化，是通过 blockmanagerMaster 来进行的。同时还会实例化 blockTranferService 和 shuffleClient
    ...
    // 动态资源分配 默认关闭。 参数 spark.dynamicAllocation.enabled 开启
    val dynamicAllocationEnabled = Utils.isDynamicAllocationEnabled(_conf)
    _executorAllocationManager =
      if (dynamicAllocationEnabled) {
        // 根据集群资源动态触发增加或者删除当前任务的资源 策略 。如果集群资源本来就比较小，任务积压严重，动态资源调整慎重开启【重量级任务会吃掉大部分集群资源，造成其他任务缺少资源无法运行】
