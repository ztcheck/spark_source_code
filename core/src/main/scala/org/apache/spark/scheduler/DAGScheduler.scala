...
class DAGScheduler(
  ...
  private def getOrCreateShuffleMapStage(
    ...
    shuffleIdToMapStage.get(shuffleDep.shuffleId) match {
      ...
      case None =>
        // Create stages for all missing ancestor shuffle dependencies. 返回一个hashSet
  ...
  def submitJob[T, U](
    ...
    /*
      这里是异步任务提交，把 event 添加到 queue 中，多线程不停的去执行任务
      taskScheduler 的 submit 方法目前是空实现
     */
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (missing.isEmpty) {
          ...
          for (parent <- missing) {
            // 重新提交stage时，会在 getMissingParentStages 中重新创建stage
            submitStage(parent)
          }
          waitingStages += stage
        }
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    /*
     如果tasks数量大于0，taskScheduler 提交 task
     调用方已经变成 taskScheduler 了，不是 dagScheduler
     */
    if (tasks.size > 0) {
